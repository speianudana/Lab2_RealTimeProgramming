defmodule MqttClient1 do
  ...
  def handle_info({:tcp,_,packet},state) do
    ...
    cond do
      decoded_packet =~ "PUBLISH" ->
        MessageStatus.save_record(decoded_packet)
  ...
  defp schedule_post(state) do
    IO.inspect("Client 1 - Checking network status...")
  ...
  def get_connection(ip, port, opts, reply) do
    case :gen_tcp.connect(ip, port, opts) do
      {:ok, socket} ->
        :gen_tcp.send(socket, MqttUtil.encode_packet_subscribe(1, ["weather sensors"]))
