defmodule Mqtt.Decoder do
  ...
  def decode(<<first_byte::bits-8, _rest::bits>> = bytes) do
    case first_byte do
      <<1::size(4), _::size(4)>> ->
        decode_connect(bytes)

      <<2::size(4), _::size(4)>> ->
        decode_connect_ack(bytes)

      <<3::size(4), _::size(4)>> ->
        decode_publish(bytes)

      <<4::size(4), _::size(4)>> ->
        decode_publish_ack(bytes)

      <<5::size(4), _::size(4)>> ->
        decode_publish_receive(bytes)

      <<6::size(4), 0::size(1), 1::size(2), 0::size(1)>> ->
        decode_publish_release(bytes)

      <<7::size(4), _::size(4)>> ->
        decode_publish_complete(bytes)

      <<8::size(4), 0::size(1), 1::size(2), 0::size(1)>> ->
        decode_subscribe(bytes)

      <<9::size(4), _::size(4)>> ->
        decode_subscribe_ack(bytes)

      <<10::size(4), 0::size(1), 1::size(2), 0::size(1)>> ->
        decode_unsubscribe(bytes)

      <<11::size(4), _::size(4)>> ->
        decode_unsubscribe_ack(bytes)

      <<12::size(4), _::size(4)>> ->
        decode_ping_request(bytes)

      <<13::size(4), _::size(4)>> ->
        decode_ping_response(bytes)

      <<14::size(4), _::size(4)>> ->
        decode_disconnect(bytes)
    end
  end
  ...
  def decode_remaining_length(<<encodedValue, rest::binary>>, accumulator, multiplier)
  ...
  def decode_connect(<<_, from_second_byte::binary>>) do
    ...
    <<4::size(16)>> <>
      ...
      <<username_flag::size(1), password_flag::size(1), will_retain::size(1), will_qos::size(2),
        will_flag::size(1), clean_session::size(1),
        0::size(1)>> <> <<keep_alive::size(16)>> <> <<rest::binary>> = message_bytes

    extract_if = fn exp, str ->
      if exp do
        extract_string(str)
      else
        {"", str}
  ...
  def decode_connect_ack(<<_, from_second_byte::binary>>) do
  ...
  def decode_publish(
         <<3::size(4), dup::size(1), qos::size(2), retain::size(1), from_second_byte::binary>>
       ) do
    {length, from_third_byte} = decode_remaining_length(from_second_byte)
    <<message_bytes::bytes-size(length), remainder::binary>> = from_third_byte
  ...
  def decode_publish_ack(<<_, from_second_byte::binary>>) do
    {2, from_third_byte} = decode_remaining_length(from_second_byte)
    <<message_bytes::bytes-size(2), remainder::binary>> = from_third_byte
  ...
  def decode_publish_release(<<_, from_second_byte::binary>>) do
    {2, from_third_byte} = decode_remaining_length(from_second_byte)
    <<message_bytes::bytes-size(2), remainder::binary>> = from_third_byte
  ...
  def decode_publish_receive(<<_, from_second_byte::binary>>) do
    {2, from_third_byte} = decode_remaining_length(from_second_byte)
    <<message_bytes::bytes-size(2), remainder::binary>> = from_third_byte
  ...
  def decode_publish_complete(<<_, from_second_byte::binary>>) do
    {2, from_third_byte} = decode_remaining_length(from_second_byte)
    <<message_bytes::bytes-size(2), remainder::binary>> = from_third_byte
  ...
  def decode_subscribe(<<_, from_second_byte::binary>>) do
    {length, from_third_byte} = decode_remaining_length(from_second_byte)
    <<message_bytes::bytes-size(length), remainder::binary>> = from_third_byte
  ...
  def extract_topics_qoses(binary) do
    extract_topics_qoses(binary, {[], []})
  end

  def extract_topics_qoses(<<>>, {acc_topics, acc_qoses}) do
    {Enum.reverse(acc_topics), Enum.reverse(acc_qoses)}
  end

  def extract_topics_qoses(binary, {acc_topics, acc_qoses}) do
    cond do
  ...
  def decode_subscribe_ack(<<_, from_second_byte::binary>>) do
    {length, from_third_byte} = decode_remaining_length(from_second_byte)
    <<message_bytes::bytes-size(length), remainder::binary>> = from_third_byte
  ...
  def extract_qos_list(binary) do
    extract_qos_list(binary, [])
  end

  def extract_qos_list(<<>>, accumulator), do: Enum.reverse(accumulator)

  def extract_qos_list(binary, accumulator) do
    {element, rest} = extract_qos(binary)
    extract_qos_list(rest, [element | accumulator])
  end

  def extract_qos(<<qos::size(8), rest::binary>>) do
    {qos, rest}
  end

  def decode_unsubscribe(<<_, from_second_byte::binary>>) do
    {length, from_third_byte} = decode_remaining_length(from_second_byte)
    <<message_bytes::bytes-size(length), remainder::binary>> = from_third_byte
  ...
  def extract_topic_list(binary), do: extract_topic_list(binary, [])

  def extract_topic_list(<<>>, accumulator), do: Enum.reverse(accumulator)

  def extract_topic_list(binary, accumulator) do
    {element, rest} = extract_topic(binary)
    extract_topic_list(rest, [element | accumulator])
  end

  def extract_topic(binary) do
    extract_string(binary)
  end

  def extract_string(<<len::size(16), topic::bytes-size(len), rest::binary>>) do
    {topic, rest}
  end

  def decode_unsubscribe_ack(<<_, from_second_byte::binary>>) do
    {2, from_third_byte} = decode_remaining_length(from_second_byte)
    <<message_bytes::bytes-size(2), remainder::binary>> = from_third_byte
  ...
  def decode_ping_request(<<_, from_second_byte::binary>>) do
  ...
  def decode_ping_response(<<_, from_second_byte::binary>>) do
    {0, from_third_byte} = decode_remaining_length(from_second_byte)
    <<_::bytes-size(0), remainder::binary>> = from_third_byte
  ...
  def decode_disconnect(<<_, from_second_byte::binary>>) do
    {0, from_third_byte} = decode_remaining_length(from_second_byte)
    <<_::bytes-size(0), remainder::binary>> = from_third_byte
